    ...
    for group_idx, (non_heads, heads) in enumerate(compound_groups):
        ...
        for head in heads:
            for i, non_head in enumerate(non_heads):
                ...
                # Full story context + target compound at the end (two last words)
                sentence = f"{prefix} {non_head} {head}"
                lines = [sentence]
                ...
                if len(word_surps) < 2:
                    print("WARNING: expected at least 2 words, got", len(word_surps), "for", sentence)

                # Take surprisal of the LAST TWO WORDS (non-head, head)
                s_non_head = word_surps[-2] if len(word_surps) >= 2 else float('nan')
...
run_experiment(compound_groups_list1, story_prefixes_list1, "results_berent&pinker_phonemes_with_spaces_experiment_2_sentences.csv")
run_experiment(compound_groups_list2, story_prefixes_list2, "results_berent&pinker_phonemes_with_spaces_experiment_3_sentences.csv")
