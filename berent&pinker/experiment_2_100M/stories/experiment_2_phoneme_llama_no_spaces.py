...
# Bow correction is kept False.
# Input to the model has no spaces.
# Translation is done at full-story level, and only then are spaces removed.
...
output_file = "results_berent&pinker/100M/results_experiment_2_phoneme_llama_no_spaces_stories.csv"
translations_file = "text_to_phonemes/compounds_with_stories_experiment_2_phoneme_llama_no_spaces_ipa.txt"
...
def word_to_ipa_no_spaces(word):
    ...
    ipa_word = compact_ipa(ipa_word)
    return ipa_word

def text_to_ipa_full_no_spaces(text):
    ...
    for m in matches:
        ...
        if left != "":
            out = g2p(left)
            left_ipa = str(out)
            left_ipa = compact_ipa(left_ipa)
            parts.append(left_ipa)

        parts.append(compact_ipa(matched_ipa))
        last = m.end()
    ...
    if right != "":
        out = g2p(right)
        right_ipa = str(out)
        right_ipa = compact_ipa(right_ipa)
...
def find_compound_ipa_span(ipa_story, ipa_non_head, ipa_head):
    ...
    head_start_raw = target_start_raw + len(ipa_non_head)

    return target_start_raw, head_start_raw, target_end_raw

def split_surprisal_by_offsets(lm, sentence, tok_scores, target_start_raw, head_start_raw, target_end_raw):
...
for group_idx, group in enumerate(stimuli_data, start=1):
    ...
    for item_idx, (non_head, story_text) in enumerate(zip(non_heads, stories), start=1):
        ...
        ipa_story = text_to_ipa_full_no_spaces(story_text)
