...
output_file = "results_berent&pinker/100M/results_experiment_2_babble_txt_char_no_spaces_stories.csv"
...
def _split_compound_surprisal_by_offsets(lm, compound_text, compound_tok_scores):
    boundary = compound_text.index(" ")
    ...
    first_tok = str(compound_tok_scores[0][0]) if len(compound_tok_scores) > 0 else ""
    start_idx = 1 if (first_tok.startswith("<") or first_tok == "UTT_BOUNDARY") else 0
    toks_only = compound_tok_scores[start_idx:]

    if len(offsets) != len(toks_only):
        raise ValueError(
            f"Offsets/token mismatch inside compound: offsets={len(offsets)} vs toks={len(toks_only)}"
        )
    ...
    for group in stimuli_data:
        ...
        for i, (non_head, story_text) in enumerate(zip(non_heads, stories)):
            ...
            surprisal_non_head, surprisal_head = _split_compound_surprisal_by_offsets(
                lm, compound, compound_tok_scores
            )
